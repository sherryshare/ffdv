#include "SysLinux2_6_26.h"
#include <stdio.h>
#include <sys/vfs.h>
#include "../../utils/PUtils.h"

namespace os{
SysLinux2_6_26::SysLinux2_6_26(void)
: m_iBlockSize(-1)
, m_iCoreNums(-1)
, m_iCPUNums(-1)
, m_iPageSize(-1)
, m_iPhysicalMem(-1)
{
	...
	if(!res.empty())
	{
		...
		{
			//2.6.26版本内核对多核的信息提示并不完整,只有逻辑CPU的数量
			pos2=res.find('\n',pos);
	...
	if(!res.empty())
	{
		//在内核2.6.26中，有九个值，其中第八个位虚拟机时间，无用；第九个未知，无用。此处只使用前7个值。
...
std::string SysLinux2_6_26::getOSVersion()
{
	return m_strOSVersion;
}

int SysLinux2_6_26::getCPUNums()
{
	if(m_iCPUNums<=0)
...
int SysLinux2_6_26::getCPUUsedRate()
{
...
int SysLinux2_6_26::getCoreNums()
{
	if(m_iCoreNums<=0)
		return addError("unsupported!");
	return m_iCoreNums;
...
std::vector<cpu_info_detail> SysLinux2_6_26::getCPUInfo()
{
	m_oCPUInfo.clear();
	return m_oCPUInfo;
}

int SysLinux2_6_26::getPhysicalMem()
{
	if(m_iPhysicalMem<=0)
		return addError("unsupported!");
...
int SysLinux2_6_26::getPhysicalUsedMem()
{
...
int SysLinux2_6_26::getBlockSize()
{
	if(m_iBlockSize<=0)
		return addError("unsupported!");
	return m_iBlockSize;
...
int SysLinux2_6_26::getPageSize()
{
	if(m_iPageSize<=0)
		return addError("unsupported!");
	return m_iPageSize;
...
int SysLinux2_6_26::getLoadAvg1()
{
...
int SysLinux2_6_26::getLoadAvg5()
{
	std::string res = readProcFile("/proc/loadavg");
	float la1,la5,la15;
	sscanf(res.c_str(),"%f %f %f ",&la1,&la5,&la15);
...
int SysLinux2_6_26::getLoadAvg15()
{
	std::string res = readProcFile("/proc/loadavg");
	float la1,la5,la15;
	sscanf(res.c_str(),"%f %f %f ",&la1,&la5,&la15);
...
int SysLinux2_6_26::getProcessNums()
{
	return addError("unsupported!");
}

std::vector<proc_info_detail> SysLinux2_6_26::getProcInfo()
{
	m_oProcInfo.clear();
	return m_oProcInfo;
}

std::string SysLinux2_6_26::readProcFile( const char * filePath )
{
...
fsinfo SysLinux2_6_26::getFSInfo()
{
	return m_ofsinfo;
}

int SysLinux2_6_26::getInNetRate()
{
...
int SysLinux2_6_26::getOutNetRate()
{
	struct timeval curtime;
	gettimeofday(&curtime,NULL);
	if(curtime.tv_sec == m_oLastCalltime.tv_sec)
